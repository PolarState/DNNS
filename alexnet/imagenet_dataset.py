...
def parse_train_archive(root: str, file: Optional[str] = None, folder: str = "train") -> None:
    ...
    for archive in archives:
        print(archive)
        extract_dir = os.path.join(os.path.splitext(archive)[0], 'raw')
        extract_archive(archive, extract_dir, remove_finished=True)
        image_paths = [os.path.join(extract_dir, image)
                  for image in os.listdir(extract_dir)]
        for image_path in image_paths:
            img = Image.open(image_path).convert('RGB')
            img = transforms.Resize(256)(img)
            img = transforms.ToTensor()(img)
            final_path = f'{os.path.splitext(archive)[0]}/{os.path.splitext(image_path)[0].split("/")[-1]}.pt'
            torch.save(img, final_path)
